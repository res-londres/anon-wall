...
class Users:
    ...
    def create_user(user_id, username):
        conn, cur = help.DB.get_conn_cur()
        cur.execute('''
            INSERT INTO users (user_id, username)
            VALUES (%s, %s)
        ''', (user_id, username))
        help.DB.close_conn_cur(conn, cur, commit=True)
        return {
            'user_id': user_id,
            'username': username,
            'is_deleted': False,
    ...
    def get_user_by_id(user_id, include_deleted=False):
        conn, cur = help.DB.get_conn_cur(cursor=RealDictCursor)
        query = 'SELECT * FROM users WHERE user_id = %s'
        if not include_deleted:
            query += ' AND is_deleted = FALSE'
        cur.execute(query, (user_id,))
        user = cur.fetchone()
        help.DB.close_conn_cur(conn, cur)
        help.Misc.convert_datetime_to_isoformat(user, ('created_at', 'deleted_at'))
        return user
...
class Posts:
    ...
    def create_post(user_id, attribution, content):
        ...
        post = cur.fetchone()
        help.DB.close_conn_cur(conn, cur, commit=True)
        help.Misc.convert_datetime_to_isoformat(post, ('created_at', 'deleted_at'))
        return post
...
class Comments:
    ...
    def create_comment(post_id, user_id, attribution, content):
        ...
        help.DB.close_conn_cur(conn, cur, commit=True)
        help.Misc.convert_datetime_to_isoformat(comment, ('created_at', 'deleted_at'))
        return comment
